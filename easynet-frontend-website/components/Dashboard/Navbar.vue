<template>
	<nav class="navbar navbar-top navbar-expand navbar-dashboard navbar-dark ps-0 pe-2 pb-0">
		<div class="container-fluid px-0">
			<div class="d-flex justify-content-between w-100 h-100" id="navbarSupportedContent">
				...
				<!-- Navbar links -->
				<ul class="navbar-nav align-items-center mb-5">
					<li class="nav-item dropdown">
						<a :class="`nav-link text-dark ${notifdata.show ? 'notification-bell' : ''} unread dropdown-toggle`" data-unread-notifications="true" href="#" role="button" data-bs-toggle="dropdown" data-bs-display="static" aria-expanded="false">
						...
						<div class="dropdown-menu dropdown-menu-lg dropdown-menu-center mt-2 py-0">
							<div class="list-group list-group-flush overflow-auto">
								...
								<a v-if="notifdata.show" href="#" class="list-group-item list-group-item-action border-bottom active">
										...
										<div class="col ps-0 ms-2">
											<div class="d-flex justify-content-between align-items-center">
													<h4 class="h6 mb-0 text-small">
													 ...
													 <p class="font-small mt-1 mb-0">{{latests.content}}</p>
												<div class="text-end">
													<small class="text-danger text-small">
														{{$moment(latests.created_at).tz("Asia/Jakarta").fromNow()}}
													</small>
												</div>
											</div>
										</div>
								...
								<a v-if="allnotifs.show" v-for="(item, index) in allnotifs.data" class="list-group-item list-group-item-action border-bottom">
									<div class="row align-items-center">
										<div  class="col ps-0 ms-2">
											<div class="d-flex justify-content-between align-items-center">
												<div>
													<nuxt-link :to="`${path}${item.route}`">
														<h4 class="h6 mb-0 text-small">
															{{item.name}}
														</h4>
													</nuxt-link>

													<p class="font-small mt-1 mb-0">{{item.content}}</p>
												</div>
												<div class="text-end">
													<small class="text-danger text-small">
														{{$moment(item.created_at).tz("Asia/Jakarta").fromNow()}}
													</small>
												</div>
											</div>
										</div>
					...
					<li class="nav-item dropdown ms-lg-3">
						<a class="nav-link dropdown-toggle pt-1 px-0" href="#" role="button" data-bs-toggle="dropdown" aria-expanded="false">
							<div class="media d-flex align-items-center">
								<img class="avatar rounded-circle" alt="Image placeholder" src="/assets/dashboard/assets/img/team/profile-picture-3.jpg">
								<div class="media-body ms-2 text-dark align-items-center d-none d-lg-block">
									<span class="mb-0 font-small fw-bold text-gray-900">{{userdata.name}}</span>
								</div>
							</div>
						</a>
...
<script>
	export default{
		props: ['auth','userdata', 'logout'],
		data(){
			return {
				notifdata:{
					show: false,
					title: 'New Messages From EasyNet Website',
					msg: ''
				},
				latests:{},
				allnotifs:{
